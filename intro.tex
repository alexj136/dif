...
\noindent This tells us that a type \texttt{a} is in the \texttt{Show} type
class when there exists a function \texttt{show} of type \texttt{a -> String}.
We can then declare such a function for a given type, for example \texttt{Bool},
as below:
\begin{lstlisting}[mathescape]
